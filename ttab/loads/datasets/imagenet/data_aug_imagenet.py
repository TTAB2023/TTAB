...
unnormalize = transforms.Compose(
    [
        transforms.Normalize(
            mean=[0.0, 0.0, 0.0],
            std=[1.0 / v for v in dataset_statistics["imagenet"]["std"]],
        ),
        transforms.Normalize(
            mean=[-v for v in dataset_statistics["imagenet"]["mean"]],
            std=[1.0, 1.0, 1.0],
        ),
    ]
)
...
preprocess = transforms.Compose(
    [
        transforms.ToTensor(),
        transforms.Normalize(
            dataset_statistics["imagenet"]["mean"],
            dataset_statistics["imagenet"]["std"],
        ),
    ]
)
...
def tr_transforms_imagenet(image):
    """
