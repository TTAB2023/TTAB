...
from ttab.loads.datasets.datasets import dataset_statistics
...
# For now, using statistics of cifar10 by default.
unnormalize = transforms.Compose(
    [
        transforms.Normalize(
            mean=[0.0, 0.0, 0.0],
            std=[1.0 / v for v in dataset_statistics["cifar10"]["std"]],
        ),
        transforms.Normalize(
            mean=[-v for v in dataset_statistics["cifar10"]["mean"]],
            std=[1.0, 1.0, 1.0],
        ),
    ]
)
...
preprocess = transforms.Compose(
    [
        transforms.ToTensor(),
        transforms.Normalize(
            dataset_statistics["cifar10"]["mean"], dataset_statistics["cifar10"]["std"]
        ),
    ]
)


def tr_transforms_cifar(image):
    """
