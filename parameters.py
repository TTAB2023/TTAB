...
def get_args():
    ...
    parser.add_argument(
        "--ckpt_path",
        default="/home/ttab/sp-ttabed/codes/code/data/pretrained_ckpts/classification/resnet26_with_head/cifar10/rn26_bn.pth",
        type=str,
    )
    ...
    # for evaluation.
    # label shift
    parser.add_argument("--label_shift_param", help="parameter to control the severity of label shift", default=None, type=float)
    parser.add_argument("--data_size", help="parameter to control the size of dataset", default=None, type=int)
    # optimal model selection
    parser.add_argument("--lr_grid", nargs="+", default=None, type=float)
    parser.add_argument("--step_ratios", nargs="+", default=[0.1, 0.3, 0.5, 0.75], help="ratios used to control adaptation step length", type=float)
    ...
    parser.add_argument('--fisher_size', default=5000, type=int, help='number of samples to compute fisher information matrix.')
    parser.add_argument('--fisher_alpha', type=float, default=1.5, help='the trade-off between entropy and regularization loss')
    # method-wise hparams
    parser.add_argument("--aug_size", default=32, help="number of per-image augmentation operations in memo and ttt", type=int)
    parser.add_argument("--threshold_ttt", default=None, help="threshold for selecting sample with low confidence to adapt in ttt", type=float)
    parser.add_argument("--threshold_shot", default=0.9, help="threshold for selecting sample with high confidence to do pseudo labelling in shot", type=float)
    parser.add_argument("--cls_par", default=0.3, help="coefficient of objective function in shot", type=float)
    parser.add_argument("--bn_prior_strength", default=None, help="single point BN adaptation in MEMO", type=int)
    parser.add_argument("--top_M", default=None, help="number of supports to store in T3A", type=int)
    parser.add_argument("--entry_of_shared_layers", default=None, help="split position of auxiliary head in TTT.")
    # metrics
    parser.add_argument("--record_preadapted_perf", default=False, help="record performance on the local batch prior to implementing test-time adaptation.", type=str2bool)

    # misc
    parser.add_argument("--grad_checkpoint", default=False, help="Trade computation for gpu space.", type=str2bool)
